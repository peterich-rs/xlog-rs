...
xlog_cli_manifest="${repo_root}/crates/xlog-cli/Cargo.toml"
xlog_cli_npm_manifest="${repo_root}/packages/mars-xlog-cli-npm/package.json"
xlog_cli_cask="${repo_root}/Casks/mars-xlog.rb"
...
assert_eq "crates/xlog-cli version" "$release_version" "$(manifest_version "$xlog_cli_manifest")"
...
assert_eq "packages/mars-xlog-cli-npm version" "$release_version" "$(awk -F'"' '/"version":/ { print $4; exit }' "$xlog_cli_npm_manifest")"
assert_eq "Casks/mars-xlog.rb version" "$release_version" "$(awk -F'"' '/^[[:space:]]*version "/ { print $2; exit }' "$xlog_cli_cask")"

assert_eq "mars-xlog -> mars-xlog-core dependency version" "$release_version" "$(dependency_version "$xlog_manifest" "mars-xlog-core")"
assert_eq "mars-xlog-cli -> mars-xlog-core dependency version" "$release_version" "$(dependency_version "$xlog_cli_manifest" "mars-xlog-core")"
